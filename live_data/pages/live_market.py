...
def render(state):
    ...
    st.caption(
        "Provider-ready marketplace search, comps, watchlist scanning, and value bands."
    )
    ...
    with tab1:
        query = st.text_input(
            "Search Query", "Caleb Williams Gold /10", key="live_search"
        )
    ...
    with tab2:
        comp_query = st.text_input(
            "Comp Query", "Caleb Williams Gold /10", key="comp_query"
        )
        if st.button("Build Comps"):
            st.session_state["comp_result"] = LiveDataActivationService.comps(
                comp_query
            )
        if "comp_result" in st.session_state:
            ...
            render_listing_cards(
                [
                    type("Obj", (), x)
                    for x in st.session_state["comp_result"].get("listings", [])
                ]
            )
