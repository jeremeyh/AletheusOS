...
def test_safe_no_action_is_approved() -> None:
    ...
    decision = governor.evaluate(plan_for(ExecutiveDecision.NO_ACTION))
...
def test_frozen_runtime_denies_plan() -> None:
    ...
    governor.set_mode(GovernorMode.FROZEN)
    ...
    assert decision.outcome is (GovernorOutcome.DENIED)
...
def test_manual_plan_is_escalated() -> None:
    ...
    assert decision.outcome is (GovernorOutcome.ESCALATED)
    assert decision.requires_manual_approval is True


def test_target_limit_throttles_plan() -> None:
    constraints = GovernorConstraints(maximum_concurrent_targets=1)

    _, _, _, governor = build_runtime(constraints=constraints)
    ...
    assert decision.outcome is (GovernorOutcome.THROTTLED)


def test_restart_budget_is_enforced() -> None:
    constraints = GovernorConstraints(maximum_restarts_per_target=1)
    ...
    assert governor.evaluate(plan).approved is True
...
def test_restart_cooldown_defers_plan() -> None:
    constraints = GovernorConstraints(restart_cooldown=timedelta(minutes=5))
    ...
    assert decision.outcome is (GovernorOutcome.DEFERRED)
...
def test_maintenance_defers_plan() -> None:
    ...
    governor.set_mode(GovernorMode.MAINTENANCE)
...
def test_unknown_target_is_rejected() -> None:
    ...
    with pytest.raises(GovernorTargetNotFoundError):
        governor.evaluate(
            plan_for(
                ExecutiveDecision.RESTART_SERVICE,
                "service.missing",
...
def test_emergency_cannot_jump_to_normal() -> None:
    ...
    governor.set_mode(GovernorMode.EMERGENCY)

    with pytest.raises(GovernorStateError):
        governor.set_mode(GovernorMode.NORMAL)


def test_snapshot_is_auditable() -> None:
    _, _, _, governor = build_runtime()

    governor.evaluate(plan_for(ExecutiveDecision.NO_ACTION))
...
def test_governor_never_executes() -> None:
    ...
    assert forbidden.isdisjoint(set(dir(governor)))
