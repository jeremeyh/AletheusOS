...
VALIDATOR = ROOT / "tools" / "validation" / "nimble" / "validate_nimble_orchestrator.py"


def test_discovery_finds_known_capabilities() -> None:
    statuses = discover_capabilities(ROOT)

    identifiers = {status.capability_id for status in statuses}
...
def test_founder_console_depends_on_primitives() -> None:
    statuses = {status.capability_id: status for status in discover_capabilities(ROOT)}

    founder = statuses["founder-console"]

    assert "react-primitives" in founder.dependencies
...
def test_build_plan_is_dependency_aware() -> None:
    ...
    capability_ids = [item.capability_id for item in plan]

    assert len(capability_ids) == len(set(capability_ids))

    assert all(item.state != "implemented" for item in plan)
    ...
    for item in plan:
        if item.state == "blocked":
            ...
            assert item.action == ("resolve dependencies")
        else:
...
def test_cli_generates_report(
    ...
    report_path = tmp_path / "nimble-orchestrator-report.json"
