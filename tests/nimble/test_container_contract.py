...
VALIDATOR = ROOT / "validate_nimble_container_contract.py"
...
def test_container_contract_requires_hardening():
    ...
    assert runtime["read_only_root_filesystem"] is True

    assert runtime["drop_all_capabilities"] is True
...
def test_validator_requires_probe_markers():
    ...
    assert "/healthz" in (module.REQUIRED_COMPOSE_MARKERS)

    assert "/readyz" in (module.REQUIRED_COMPOSE_MARKERS)
