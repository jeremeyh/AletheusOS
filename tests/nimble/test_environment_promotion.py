...
def test_staging_precedes_production() -> None:
    ...
    assert contract["promotion_policy"]["staging_before_production"] is True


def test_production_requires_approval() -> None:
    contract = load_contract()

    assert contract["environments"]["production"]["requires_manual_approval"] is True


def test_production_requires_rollback() -> None:
    contract = load_contract()

    assert contract["environments"]["production"]["rollback_required"] is True


def test_health_checks_are_required() -> None:
    contract = load_contract()

    assert contract["environments"]["staging"]["health_probe_required"] is True

    assert contract["environments"]["production"]["health_probe_required"] is True
