...
def test_text_supports_instrument_roles() -> None:
    ...
    assert "font.family.instrument" in text


def test_visualization_primitives_exist() -> None:
    assert 'data-nimble-primitive="meter"' in read("Meter.tsx")

    assert 'data-nimble-primitive="signal"' in read("Signal.tsx")


def test_instrument_supports_density_profiles() -> None:
    instrument = read("Instrument.tsx")
...
def test_reserved_instrument_identity_is_immutable() -> None:
    reserved = read("reserved.tsx")

    assert 'id="instrument.aletheus-index"' in reserved

    assert 'label="Aletheus Index™"' in reserved

    assert 'id="instrument.thorx"' in reserved

    assert 'label="THORᵡ"' in reserved
...
def test_primitive_validator_passes() -> None:
    result = subprocess.run(
        [
            "python",
            str(ROOT / "validate_nimble_primitive_experience.py"),
        ],
        cwd=ROOT,
        capture_output=True,
        text=True,
