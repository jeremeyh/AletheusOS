...
def test_ledger_uses_canonical_transtemporal_engine():
    ...
    assert ledger.transtemporal.health()["name"] == ("Transtemporal Engine™")


def test_time_travel_remains_compatibility_alias():
    ledger = ConstitutionalLedger()

    compatibility_engine = TimeTravel(ledger.institutional_events)
...
def test_transtemporal_lineage_traces_causality():
    ...
    assert [event.event_id for event in lineage] == [
        "EVENT-ROOT",
        "EVENT-ASSESSMENT",
        "EVENT-DECISION",
    ]
...
def test_transtemporal_provenance_is_inspectable():
    ...
    provenance = ledger.temporal_provenance(event.event_id)

    assert provenance["found"] is True
    assert provenance["source_identity"] == ("aletheus.watch_tower")
...
def test_unknown_provenance_is_explicit():
    ...
    provenance = ledger.temporal_provenance("EVENT-DOES-NOT-EXIST")
