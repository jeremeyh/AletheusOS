...
def test_legacy_runtime_migration_layers_do_not_return():
    ...
    existing = [str(path.relative_to(REPOSITORY_ROOT)) for path in forbidden_paths if path.exists()]

    assert not existing, (
        "Retired runtime migration layers have returned: "
        + ", ".join(existing)
    )
...
def test_no_legacy_runtime_migration_references():
    ...
    for path in python_sources():
        ...
        for symbol in forbidden_symbols:
            if symbol in source:
                violations.append(
                    f"{path.relative_to(REPOSITORY_ROOT)}: {symbol}"
                )

    assert not violations, (
        "Legacy runtime migration references remain:\n"
