...
def render(state):
    ...
    labels = [
        f"{row_value(a, 'asset_id', '')} — {row_value(a, 'player', 'Unknown')} — {row_value(a, 'brand', '')}"
        for a in assets
    ]
    ...
    st.caption(
        f"{row_value(asset, 'year', '')} {row_value(asset, 'brand', '')} • {row_value(asset, 'set_name', '')} • {row_value(asset, 'parallel', '')}"
    )
    ...
    c4.metric(
        "ROI",
        f"{roi_percent(row_value(asset, 'purchase_price', 0), row_value(asset, 'current_value', 0)):.1f}%",
    )
    ...
    with tabs[0]:
        ...
        if img:
            try:
                ...
                st.info("Image path stored but not displayable.")
        else:
