...
shutil.copy2(CORE, backup_dir / "core.py")

if ADAPTER.exists():
    shutil.copy2(
        ADAPTER,
        backup_dir / "prediction_adapter.py",
    )
...
old_binding = "        self.prediction = prediction_core"
...
print("Genesis 8 prediction adapter installed.")
print(f"Backup: {backup_dir.relative_to(ROOT)}")
print(f"Created: {ADAPTER.relative_to(ROOT)}")
print(f"Updated: {CORE.relative_to(ROOT)}")
