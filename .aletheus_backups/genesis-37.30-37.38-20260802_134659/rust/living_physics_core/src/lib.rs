...
pub fn step_body(body:JsValue,tx:f32,ty:f32,tz:f32,field:JsValue,dt:f32)->Result<JsValue,JsValue>{
 ...
 let stress = 0.55 * c(b.urgency) + 0.45 * c(b.contradiction);
 let e = (f.elasticity * (0.65 + 0.35 * c(b.confidence)) * (1.0 - 0.55 * stress)).max(0.05);
 let g = f.gravity * b.mass.max(0.0) * (0.5 + 0.5 * c(b.confidence));
 for _ in 0..4{
  ...
  b.vx+=(e*(tx-b.x)+dx)*h; b.vy+=(e*(ty-b.y)+dy)*h; b.vz+=(e*(tz-b.z)+dz-g * 0.0005)*h;
