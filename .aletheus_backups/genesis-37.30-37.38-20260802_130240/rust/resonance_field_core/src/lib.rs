...
pub fn evaluate(natural:f64,driving:f64,zeta:f64,coupling:f64,coherence:f64,vitality:f64,latency:f64,target:f64)->Result<JsValue,JsValue>{
 ...
 let gain=coupling/d;
 let damped=natural*(1.0-zeta.clamp(0.0,.999999).powi(2)).sqrt();
 let practical=((.55*coherence.clamp(0.0,1.0)+.45*vitality.clamp(0.0,1.0))*target/latency.max(target)).clamp(0.0,1.0);
