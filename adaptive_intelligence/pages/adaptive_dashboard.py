...
def render(state):
    ...
    with tabs[0]:
        ...
        rec_type = st.selectbox(
            "Recommendation Type", ["Buy", "Sell", "Grade", "Watch", "Pass"]
        )
    ...
    with tabs[1]:
        ...
        status_panel(
            "Sample THORᵡ Explanation",
            AdaptiveIntelligenceService.explainer.explain_thorx(sample),
        )
    ...
    with tabs[2]:
        ...
        status_panel(
            "Grade vs Raw",
            AdaptiveIntelligenceService.simulation.grade_vs_raw(
                raw, grading_cost, gem, prob
            ),
        )
    ...
    with tabs[3]:
        status_panel(
            ...
            AdaptiveIntelligenceService.quality.snapshot(),
        )
    ...
    with tabs[5]:
        status_panel(
            "Plugin Registry™",
            [p.__dict__ for p in AdaptiveIntelligenceService.plugins.seed_defaults()],
        )

    with tabs[6]:
        status_panel(
            "Deployment Profiles™", AdaptiveIntelligenceService.deployment.all()
        )

    with tabs[7]:
        status_panel(
            "Platform Governance™", AdaptiveIntelligenceService.governance.validate()
        )
        status_panel(
            "Compatibility Report",
            AdaptiveIntelligenceService.governance.compatibility_report(),
        )
