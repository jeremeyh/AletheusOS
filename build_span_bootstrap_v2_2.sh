...
class SPANBootstrap:
    ...
    def run(self, project_root: str | None = None) -> "SPANBootstrap":
        ...
        ``project_root`` is accepted for compatibility with older callers.
        The canonical Bootstrap v2 composition root resolves its components
        from the active project environment and therefore does not require
        the value directly.
        """
        self.initialize()
        return self
    ...
    @property
    def ready(self) -> bool:
        """Legacy compatibility alias for ``initialized``."""
        return self.initialized

    def status(self) -> BootstrapReport:
        """Legacy compatibility accessor for the current report."""
        return self.report

    @property
    def pipeline_result(self) -> Any:
        """
        Legacy compatibility alias.

        Older integration code expected bootstrap.run(...) to return an
        object exposing ``pipeline_result``. Bootstrap v2 exposes ``pipeline``.
        """
...
assert bootstrap.run(".") is bootstrap
assert bootstrap.ready is True
assert bootstrap.status() is bootstrap.report
assert bootstrap.pipeline_result is bootstrap.pipeline
...
print("Legacy run compatibility: OK")
print("Legacy ready compatibility: OK")
print("Legacy status compatibility: OK")
print("Legacy pipeline_result compatibility: OK")
...
echo
echo "Updating SPAN integration for Bootstrap v2 compatibility..."

INTEGRATION_TARGET="aletheus/span/integration.py"
if [[ -f "$INTEGRATION_TARGET" ]]; then
    cp "$INTEGRATION_TARGET" "$BACKUP_DIR/integration.py"
fi

cat > "$INTEGRATION_TARGET" <<'PY'
from __future__ import annotations
...
class SPANIntegrationResult:
    """Combined SPAN pipeline and rule-evaluation result."""
...
class SPANIntegration:
    """Coordinates SPAN bootstrap execution with rule evaluation."""
    ...
    def run(
        ...
        """
        Initialize SPAN, resolve the composed pipeline, and evaluate rules.

        This bridge supports both the legacy ``run(project_root)`` bootstrap
        contract and the canonical Bootstrap v2 ``initialize()`` contract.
        """

        boot = (
            self.bootstrap.run(project_root)
            if hasattr(self.bootstrap, "run")
            else self.bootstrap.initialize() or self.bootstrap
        )

        pipeline = getattr(
            boot,
            "pipeline",
            getattr(boot, "pipeline_result", None),
        )
...
python -m py_compile "$INTEGRATION_TARGET"

python - <<'PY'
from aletheus.span.bootstrap import SPANBootstrap
from aletheus.span.integration import SPANIntegration, SPANIntegrationResult

bootstrap = SPANBootstrap(strict=False)
integration = SPANIntegration(bootstrap=bootstrap)
result = integration.run(".")

assert isinstance(result, SPANIntegrationResult)
assert bootstrap.initialized
assert result.pipeline_result is bootstrap.pipeline
assert result.rule_findings is not None

print("Integration import: OK")
print("Bootstrap v2 bridge: OK")
print("Pipeline resolution: OK")
print("Rule evaluation: OK")
PY
...
echo
echo "=============================================="
echo "Genesis 13.5.2 SPAN Bootstrap v2 + Integration Bridge"
echo "Installed and fully validated successfully."
echo "=============================================="
echo
echo "Created:"
echo "  $TARGET"
echo "  aletheus/span/integration.py"
echo
echo "Updated:"
