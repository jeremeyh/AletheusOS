...
class RepairPlanner:
    ...
    def plan(
        self, target: ScanManifest, source: ScanManifest | None = None
    ) -> RepairPlan:
        plan = RepairPlan(
                ...
                if source_record.file_class in {FileClass.CACHE, FileClass.GENERATED}:
                    plan.actions.append(
                        Action(
                            ActionKind.SKIP,
                            path,
                            f"source-only {source_record.file_class.value}",
                        )
                    )
                elif self.policy.is_known_orphan(path, source_record.size):
                    plan.actions.append(
                        Action(ActionKind.SKIP, path, "source-only known orphan")
                    )
                else:
                    plan.actions.append(
                        Action(
            ...
            if target_record and not source_record:
                plan.actions.append(
                    Action(
                        ActionKind.KEEP,
                        path,
                        "exists only in canonical target",
                        target_sha256=target_record.sha256,
                    )
                )
                continue
            ...
            if target_record.sha256 == source_record.sha256:
                ...
                plan.actions.append(
                    Action(
                        ActionKind.QUARANTINE,
                        path,
                        f"target {target_record.file_class.value} differs",
                        source_record.sha256,
                        target_record.sha256,
                    )
                )
                ...
                plan.actions.append(
                    Action(
                        ActionKind.CONFLICT,
                        path,
                        "same path has different content; target preserved",
                        source_record.sha256,
                        target_record.sha256,
                    )
                )
