...
class RepairExecutor:
    def execute(
        ...
        *,
        ...
        quarantine = (
            target / ".aletheus_restore_points" / "repository_self_repair" / timestamp()
        )
        result = ExecutionResult(
            started_at=started, completed_at=started, dry_run=dry_run
        )
            ...
            try:
                ...
                if action.kind is ActionKind.COPY:
                    ...
                    result.applied.append(
                        {"action": "copy", "path": action.relative_path}
                    )
                    ...
                    result.applied.append(
                        {"action": operation, "path": action.relative_path}
                    )
        ...
        if archive_source and source is not None and not result.failed:
            archive_root = (
                archive_root or target.parent / "repository_archives"
            ).resolve()
            ...
            if remove_source:
                if not archive_source:
                    result.failed.append(
                        {
                            "path": str(source),
                            "error": "source removal requires archive",
                        }
                    )
                elif not dry_run:
