...
class UniversalOpportunityEngine:
    ...
    def evaluate(self, asset):
        ...
        signals = {
            "identity": self.identity.evaluate(asset).get("confidence", 0),
            "market": min(
                100, max(0, int(self.market.evaluate(asset).get("value_gap", 0) / 10))
            ),
            "scarcity": self.scarcity.evaluate(asset).get("scarcity", 0),
        }

        score = self.scoring.calculate(signals)

        return {
            "score": score,
            "decision": self.decision.decide(score),
            "signals": signals,
        }
