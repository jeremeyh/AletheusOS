...
class RegistryFederationGraph:
    ...
    def build(self):
        ...
        registry_map = self.load_json("registry_map.json")

        report = self.load_json("wiring_report.json")

        for registry in registry_map:
            self.add_node(registry, "registry")

        for item in report:
            if isinstance(item, dict):
                name = item.get("name") or item.get("engine") or item.get("component")

                if name:
                    self.add_node(name, "component", item)

                    self.add_edge(name, "runtime", "attached")
