...
class RegistryFederationGovernance:
    ...
    def evaluate(self):
        ...
        if not nodes:
            findings.append(
                {"severity": "warning", "finding": "No federation graph loaded"}
            )
        ...
        if not edges:
            findings.append(
                {
                    "severity": "warning",
                    "finding": "No federation relationships detected",
                }
            )

        decision = {
            "timestamp": time.time(),
            "status": "healthy" if not findings else "review_required",
...
if __name__ == "__main__":
    ...
    print(json.dumps(governance.evaluate(), indent=2))
