...
class JobQueue:
    ...
    def run_next(self) -> RuntimeJob | None:
        ...
        job = queued[0]
        job.status = "running"
        context = self.runtime.commands.dispatch(
            job.command, job.payload, application=job.application
        )
        job.result = context.to_dict()
        job.status = "failed" if context.errors else "completed"
