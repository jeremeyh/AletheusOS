...
class AnchorDependencyGraph:
    ...
    def register_dependency(self, anchor, requires):

        self.dependencies.setdefault(anchor, [])

        if requires not in self.dependencies[anchor]:
            self.dependencies[anchor].append(requires)

    def dependencies_for(self, anchor):

        return self.dependencies.get(anchor, [])
        ...
        for anchor, deps in self.dependencies.items():
            for dependency in deps:
                if dependency not in self.registry.list():
                    violations.append(
                        {"anchor": anchor, "missing_dependency": dependency}
                    )
    ...
    def can_detach(self, anchor):
        ...
        return {"safe": len(blockers) == 0, "dependent_anchors": blockers}
