...
class AnchorPredictiveIntelligence:
    def __init__(self, intelligence, learning, healing):
    ...
    def predict(self, anchor):
        ...
        failures = len([item for item in history if item["outcome"] == "failure"])

        risk = self.calculate_risk(score["intelligence_score"], failures)
    ...
    def calculate_risk(self, score, failures):
    ...
    def predict_all(self, anchors):

        return [self.predict(anchor) for anchor in anchors]
