...
class EngineRegistry:
    def __init__(self) -> None:
        self.engines: dict[str, Callable[[RuntimeContext], RuntimeContext]] = {}

    def register(
        self, name: str, handler: Callable[[RuntimeContext], RuntimeContext]
    ) -> None:
        self.engines[name] = handler

    def run(self, name: str, context: RuntimeContext) -> RuntimeContext:
        if name not in self.engines:
            context.add_error(f"Engine not registered: {name}")
            return context
