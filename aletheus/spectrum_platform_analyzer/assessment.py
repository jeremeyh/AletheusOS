    ...
    def assess(self, root: str):
        ...
        for hotspot in hotspot_analyzer.large_components(root):
            finding = PlatformFinding(
                ...
                description=(f"{hotspot['file']} contains {hotspot['lines']} lines."),
                component=hotspot["file"],
                recommendation=(
                    "Reduce responsibility by extracting focused managers/components."
                ),
            )
        ...
        for violation in boundary_analyzer.analyze(root):
            finding = PlatformFinding(
                ...
                description=(f"{violation['module']} imports {violation['import']}"),
                component=violation["module"],
                recommendation=(
                    "Respect subsystem boundaries or introduce shared contracts."
                ),
            )
